...
func runProtocolCommand(command string, args []string) int {
...
func runBundle(args []string) int {
	...
	if *output == "" {
		...
		fmt.Fprintln(os.Stderr, "Usage: remote-ui bundle [-src <binary>] -o <output> <site-dir>")
		return 1
	}
...
func runCat(args []string) int {
	if len(args) == 0 {
		...
		fmt.Fprintln(os.Stderr, "Usage: remote-ui cat <file>")
		return 1
	}
...
func runCp(args []string) int {
	if len(args) < 2 {
		...
		fmt.Fprintln(os.Stderr, "Usage: remote-ui cp <pattern> <dest-dir>")
		return 1
	}
