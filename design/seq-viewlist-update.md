...
When the ViewList variable has `wrapper=lua.ViewList` property (set via path properties like `contacts?itemWrapper=ContactPresenter`):
...
4. Views render using ViewListItem type's viewdef (or custom type if `itemWrapper=` specified)
